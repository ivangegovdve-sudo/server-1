...
class ImportService {
	...
	public function import($source, AddressBookImpl $addressBook, ContactsImportOptions $options): Generator {
		...
		return match ($options->getFormat()) {
			'ical' => $this->importProcess($source, $addressBook, $options, $this->importText(...)),
			'jcal' => $this->importProcess($source, $addressBook, $options, $this->importJson(...)),
			'xcal' => $this->importProcess($source, $addressBook, $options, $this->importXml(...)),
	 ...
	 * @return Generator<int|string, VCalendar|array{VEVENT: int, VTODO: int, VJOURNAL: int}, mixed, void>
	 */
	public function importXml($source, ?ContactsImportOptions $options = null): Generator {
		if (!is_resource($source)) {
			throw new InvalidArgumentException('Invalid import source must be a file resource');
		}
	 ...
	 *
	 * @param resource $source
	 * @param ContactsImportOptions|null $options
	 *
	 * @return Generator<int|string, VCalendar|array{VEVENT: int, VTODO: int, VJOURNAL: int}, mixed, void>
	 */
	public function importJson($source, ?ContactsImportOptions $options = null): Generator {
		if (!is_resource($source)) {
			throw new InvalidArgumentException('Invalid import source must be a file resource');
		}
	...
	public function importProcess($source, AddressBookImpl $addressBook, ContactsImportOptions $options, callable $generator): Generator {
		$addressBookId = $addressBook->getKey();
		$addressBookUri = $addressBook->getUri();
		$principalUri = $addressBook->getPrincipalUri();
		foreach ($generator($source, $options) as $key => $value) {
			...
			$objectId = $this->backend->getAddressBookObjectByUID($principalUri, $uid, $addressBookUri);
			$objectData = $vObject->serialize();
			try {
				if ($objectId === null) {
					$objectId = UUIDUtil::getUUID();
					$this->backend->createAddressBookObject(
						$addressBookId,
						$objectId,
						$objectData
					);
					...
					if ($options->getSupersede()) {
						$this->backend->updateAddressBookObject(
							$addressBookId,
							$oid,
							$objectData
						);
