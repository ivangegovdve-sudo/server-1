...
class UsersController extends AUserDataOCSController {
	...
	public function editUserMultiField(
		...
		if (!$isSelf && empty($permittedFields)) {
			// OCSForbiddenException (rather than the older OCSException pattern
			// in editUser): caller is authenticated but lacks permission.
			throw new OCSForbiddenException('Insufficient permissions to edit this user');
		}

		// Permission gate is shared with editUser; value-level validation runs
		// only when the field is permitted.
		$errors = [];
